                        ...
                        <form id="form" action="${pageContext.request.contextPath}/user/#" method="post">
                            <table id="dataList"
                                ...
                                <c:forEach items="${pageInfo.list}" var="user" varStatus="status">
                                    <tr>
                                        <td><label><input name="ids" type="checkbox" value="${user.id}"></label></td>
                                        <td>${status.count}</td>
                                        <td>${user.username}</td>
                                        <td>${user.email}</td>
                                        <td>${user.phoneNum}</td>
                                        <c:if test="${user.roleNames==null || user.roleNames==''}">
                                            <td>无</td>
                                        </c:if>
                                        <c:if test="${user.roleNames!=null}">
                                            <td>${user.roleNames}</td>
                                        </c:if>
                                        <td class="text-center">
                                            <button type="button" class="btn bg-olive btn-xs"
                                                    onclick="location.href='${pageContext.request.contextPath}/user/queryUserById?id='+${user.id}">
                                                编辑
                                            </button>
                                            <button type="button" class="btn bg-olive btn-xs"
                ...
                <div class="box-footer">
                    <div class="pull-left">
                        <div class="form-group form-inline">
                            ...
                            <label for="changePageSize"></label>
                    ...
                    <div class="box-tools pull-right">
                        <ul class="pagination">
                            <li>
                                <a href="${pageContext.request.contextPath}/user/findAllUser?pageNum=1&pageSize=${pageInfo.pageSize}"
                                   aria-label="Previous">首页</a>
                            </li>
                            <li>
                                <a href="${pageContext.request.contextPath}/user/findAllUser?pageNum=${pageInfo.pageNum-1}&pageSize=${pageInfo.pageSize}">上一页</a>
                            </li>
                            <c:forEach begin="1" end="${pageInfo.pages}" var="pageNow">
                                <li>
                                    <a href="${pageContext.request.contextPath}/user/findAllUser?pageNum=${pageNow}&pageSize=${pageInfo.pageSize}">${pageNow}</a>
                                </li>
                            </c:forEach>
                            <c:if test="${pageInfo.pageNum == pageInfo.pages}">
                                <li>
                                    <a href="${pageContext.request.contextPath}/user/findAllUser?pageNum=${pageInfo.pageNum}&pageSize=${pageInfo.pageSize}">下一页</a>
                                </li>
                            </c:if>
                            <c:if test="${pageInfo.pageNum != pageInfo.pages}">
                                <li>
                                    <a href="${pageContext.request.contextPath}/user/findAllUser?pageNum=${pageInfo.pageNum+1}&pageSize=${pageInfo.pageSize}">下一页</a>
                                </li>
                            </c:if>
                            <li>
                                <a href="${pageContext.request.contextPath}/user/findAllUser?pageNum=${pageInfo.pages}&pageSize=${pageInfo.pageSize}"
...
<script type="text/javascript">
    ...
    $("#batchDelete").on("click", function () {
        if (confirm("你确定要批量删除吗?")) {
            ...
            // console.log(ids)
            //把获取的ids数组提交到后台
            location.href = "${pageContext.request.contextPath}/user/batchDeleteUser?ids=" + ids;
        }
    });
    //改变分页的每页显示记录数
    function changePageSize() {
        ...
        location.href = "${pageContext.request.contextPath}/user/findAllUser?pageNum=1&pageSize=" + pageSize + "&username=" + v;
    }
    //搜索
    $("#search").on("click", function () {
        var v = $("#search_input").val();
        location.href = "${pageContext.request.contextPath}/user/findAllUser?username=" + v;
    });
